            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {proyector && materias.length > 0 && (
                    <div className="space-y-2">
                        ...
                        <DropdownList
                            //options={materias.map(m => `${m.carrera} | ${m.cod_materia} | ${m.materia}`)}
                            options={materias.map(m => ({
                                ...
                                carrera: m.carrera, // Almacena también la carrera
                                materia: m.materia, // Almacena también la materia
                            }))}
                            placeholder="Selecciona una materia"
                            onSelect={(cod_materia) => {
                                // Aquí puedes manejar el cod_materia seleccionado y cualquier otra lógica adicional
                                setMateria(cod_materia); // Almacena solo el cod_materia
                            }} 
                        />
                    </div>
                )}
