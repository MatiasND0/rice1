...
CREATE TABLE IF NOT EXISTS `laboratorioTesting`.`retiros-devoluciones` (
  `id_retiro-devolucion` INT NOT NULL AUTO_INCREMENT,
  `f_retiro` DATETIME NOT NULL,
  `f_devolucion` DATETIME NULL,
  `reservas_id_reserva` INT NOT NULL,
  `reservas_proyectores_nro_inv` INT NOT NULL,
  `reservas_usuario_materia_usuarios_id_usuario` INT NOT NULL,
  `reservas_usuario_materia_materias_cod_materia` INT NOT NULL,
  `aula` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`id_retiro-devolucion`, `reservas_id_reserva`, `reservas_proyectores_nro_inv`, `reservas_usuario_materia_usuarios_id_usuario`, `reservas_usuario_materia_materias_cod_materia`),
  INDEX `fk_retiros-devoluciones_reservas1_idx` (`reservas_id_reserva` ASC, `reservas_proyectores_nro_inv` ASC, `reservas_usuario_materia_usuarios_id_usuario` ASC, `reservas_usuario_materia_materias_cod_materia` ASC) VISIBLE,
  UNIQUE INDEX `id_retiro-devolucion_UNIQUE` (`id_retiro-devolucion` ASC) VISIBLE,
  CONSTRAINT `fk_retiros-devoluciones_reservas1`
    FOREIGN KEY (`reservas_id_reserva` , `reservas_proyectores_nro_inv` , `reservas_usuario_materia_usuarios_id_usuario` , `reservas_usuario_materia_materias_cod_materia`)
    REFERENCES `laboratorioTesting`.`reservas` (`id_reserva` , `proyectores_nro_inv` , `usuario_materia_usuarios_id_usuario` , `usuario_materia_materias_cod_materia`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);
