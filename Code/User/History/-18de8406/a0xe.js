...
module.exports.withdrawals = (req, res) => {
    ...
    const deleteSql = 'DELETE FROM reservas WHERE id_reserva = ?';
    connection.query(deleteSql, [reservaId], (error, results) => {
        if (error) {
            console.error('Error al eliminar la reserva:', error);
            return res.status(500).json({ error: 'Error al eliminar la reserva' });
        }
        ...
        // Registrar el retiro en la tabla retiro_devolucion
        const aula = req.body.aula; // Se asume que el aula se pasa en el cuerpo de la solicitud
        if (!aula) {
            return res.status(400).json({ error: 'El aula es requerida para el retiro' });
        }

        // Consulta SQL para insertar el retiro
        const insertSql = 'INSERT INTO retiro_devolucion (aula, fecha) VALUES (?, ?)';
        connection.query(insertSql, [aula, fechaActual], (insertError, insertResults) => {
            if (insertError) {
                console.error('Error al registrar el retiro-devolución:', insertError);
                return res.status(500).json({ error: 'Error al registrar el retiro-devolución' });
            }

            // Respuesta exitosa
            res.status(200).json({ message: 'Reserva eliminada y retiro registrado exitosamente' });
        });
    });
};