...
module.exports.bookProy = (req, res) => {
    const { fecha, turno, nro_inv, username, cod_materia, aula, role } = req.body;
    ...
    if (role === 1)
    {
        getUserIdSql = `
        SELECT u.id_usuario 
        FROM laboratorioTesting.usuarios
        WHERE u.username = ? ;
        `;  
    }
    ...
    connection.query(getUserIdSql, [username, cod_materia], (err, userResults) => {
        ...
        let usuarioId = userResults[0].id_usuario;
...
module.exports.withdrawals = (req, res) => {
    const { aula, id_reserva } = req.body;

    if (!aula || !id_reserva) {
        return res.status(400).json({ error: 'Aula y ID de reserva son necesarios' });
    }

    const sql = `
        INSERT INTO \`laboratorioTesting\`.\`retiros_devoluciones\` 
        (\`f_retiro\`, \`aula\`, \`reservas_id_reserva\`)
        VALUES 
        (NOW(), ?, ?)
    `;
    
    connection.query(sql, [aula, id_reserva], (error, results) => {
        if (error) {
            console.error('Error al realizar el retiro:', error);
            return res.status(500).json({ error: 'Error al realizar el retiro' });
        }
