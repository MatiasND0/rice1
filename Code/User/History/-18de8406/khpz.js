...
module.exports.bookProy = (req, res) => {
    const { fecha, turno, nro_inv, username, cod_materia, hdmiRequired } = req.body;

    // Validar que se recibieron los datos necesarios
    if (!fecha || !turno || !nro_inv || !username || !cod_materia) {
    ...
    connection.query(getUserIdSql, [username, cod_materia], (err, userResults) => {
        ...
        // Verificar si ya existe una reserva para el proyector en la fecha y turno seleccionados
        const checkConflictSql = `
            SELECT * FROM laboratorioTesting.reservas
            WHERE proyectores_nro_inv = ? AND fecha = ? AND turno = ?;
        `;

        connection.query(checkConflictSql, [nro_inv, fecha, turno], (err, conflictResults) => {
            if (err) {
                console.error('Error al verificar conflictos de reservas:', err);
                return res.status(500).json({ error: 'Error al verificar conflictos de reservas' });
            }

            // Si ya hay una reserva, devolver un error
            if (conflictResults.length > 0) {
                return res.status(400).json({ error: 'El proyector ya está reservado en esta fecha y turno' });
            }

            // Consulta SQL para insertar una nueva reserva
            const sql = `
                INSERT INTO laboratorioTesting.reservas (proyectores_nro_inv, fecha, turno, usuario_materia_usuarios_id_usuario, usuario_materia_materias_cod_materia) 
                VALUES (?, ?, ?, ?, ?)
            `;

            // Ejecutar la consulta de reserva
            connection.query(sql, [nro_inv, fecha, turno, usuarioId, cod_materia], (err, results) => {
                if (err) {
...
module.exports.bookedProy = (req, res) => {
    ...
    if (role === 1) {
        sql = `
            SELECT r.*, u.*, m.materia, p.cod_rec
            FROM reservas r
            JOIN usuario_materia um ON r.usuario_materia_usuarios_id_usuario = um.usuarios_id_usuario
            JOIN usuarios u ON um.usuarios_id_usuario = u.id_usuario
            JOIN materias m ON um.materias_cod_materia = m.cod_materia
            JOIN proyectores p ON r.proyectores_nro_inv = p.nro_inv;
        `;
    }
