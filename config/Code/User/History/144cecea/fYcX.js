    ...
    try {
        connection.query(sqlUser, [username], (err, results) => {
            ...
            bcrypt.compare(password, user.password, (err, isMatch) => {
                ...
                if (user.rol === 1) {
                    sqlMaterias = `SELECT cod_materia, materia, carrera FROM materias`;
...
module.exports.register = (req, res) => {
    const { username, password, role, nro_telefono, nombre, apellido, cod_materias, dni } = req.body;

    // Validaciones simples
    if (!username || !password || !role || !nro_telefono || !nombre || !apellido || !cod_materias || !dni) {
