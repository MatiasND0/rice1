    ...
    try {
        connection.query(sqlUser, [username], (err, results) => {
            ...
            bcrypt.compare(password, user.password, (err, isMatch) => {
                ...
                let sqlMaterias = `
                    SELECT m.cod_materia, m.materia, m.carrera 
...
module.exports.getUsers = (req, res) => {
    const userRole = req.params.role; // Suponiendo que el ID se pasa como parámetro en la URL

    if (!userId) {
        return res.status(400).json({ message: 'El usuario no es administrador' });
    }
