...
export function Reservas({ parsedToken }) {
  ...
  const handleAction = async () => {
    ...
    if (actionType === 'delete') {
      ...
      method = 'POST';
      body = JSON.stringify({ deleteId, aula, fechaActual}); // Se agrega el aula en el cuerpo de la solicitud
    } else if (actionType === 'return') {
      url = `http://${IP}:3000/withdraws/${deleteId}`;
      method = 'PATCH';
    }

    try {
      const response = await fetch(url, { method, body: body ? body : undefined });
