...
import DropdownList from "./DropdownList"; // Importa tu componente DropdownList
    ...
    return (
        <div className="w-full max-w-3xl mx-auto p-6 bg-white rounded-lg shadow-md">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {hora && (
                    <div className="space-y-2">
                        ...
                        >
                            ...
                            {proyectoresDisponibles.map((proyector, index) => (
                                <option key={index} value={proyector.nro_inv}>
                                    {proyector.nro_inv}
                                </option>
                            ))}
                        </select>
                ...
                {hora && (
                    <div className="space-y-2">
                        <label className="block text-sm font-medium text-gray-700">Selecciona una materia:</label>
                        <DropdownList
                            options={materias.map(materia => ({
                                value: materia.cod_materia, // Este es el valor que se almacenará
                                label: `${materia.carrera} - ${materia.cod_materia} - ${materia.materia}` // Este es el texto que se mostrará
                            }))}
                            placeholder="Selecciona una materia"
                            onSelect={setMateria} // Almacena solo el cod_materia
                        />
                    </div>
                )}
                ...
                <Button type="submit">Reservar Proyector</Button>
            </form>
        </div>
    );
}
