...
module.exports.withdrawals = (req, res) => {
    ...
    const sql = `
    INSERT INTO \`laboratorioTesting\`.\`retiros-devoluciones\` (\`f_retiro\`, \`aula\`, \`reservas_id_reserva\` )
    VALUES 
    (CURDATE(), ?)
  `;
  
  connection.query(sql, [aula, idReserva, nroInventario, idUsuario, codMateria], (err, results) => {
    if (err) {
      console.error('Error al insertar retiro:', err);
      return;
    }
    console.log('Retiro registrado con éxito:', results);
  });
};